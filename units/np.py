...
class NP:
    ...
    def get_np_damage_multiplier(self, np_level=1, overcharge_level=1, new_id=None):
        ...
        effects = self.get_np_values(np_level, overcharge_level, new_id)
        
        for effect in effects:
            func_type = effect.get('funcType', '')
            if any(dmg_type in func_type.lower() for dmg_type in ['damagenp', 'damage']):
                return effect.get('parameters', {}).get('value', 0) / 1000.0
    ...
    def get_np_special_damage(self, np_level=1, overcharge_level=1, new_id=None):
        ...
        for effect in effects:
            func_type = effect.get('funcType', '')
            if 'individual' in func_type.lower() or 'sum' in func_type.lower():
                params = effect.get('parameters', {})
                return {
                    'base_damage': params.get('value', 0) / 1000.0,
                    'correction_damage': params.get('value2', 0) / 1000.0,
                    'correction_multiplier': params.get('correction', 0) / 1000.0,
                    'target': params.get('target', 0),
                    'target_list': params.get('targetlist', [])
                }
